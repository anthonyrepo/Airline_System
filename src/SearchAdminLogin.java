...
public class SearchAdminLogin {

	private static JFrame searchAdminLogInFrame;
	...
	private String filePath = "searchAdminLogin.txt"; // File where account details will be stored
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					SearchAdminLogin window = new SearchAdminLogin();
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void initialize() {
		searchAdminLogInFrame = new JFrame("Search Engine Administrator Sign-In Page");
		searchAdminLogInFrame.setVisible(true);
		searchAdminLogInFrame.setTitle("Search Engine Administrator Sign-In Page");
		searchAdminLogInFrame.setBounds(100, 100, 730, 510);
		//searchAdminLogInFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		searchAdminLogInFrame.getContentPane().setLayout(null);
		...
		searchAdminLogInFrame.getContentPane().add(signinWindow);
		...
		signinWindowBtn.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				if (found == true) {
					JOptionPane.showMessageDialog(searchAdminLogInFrame, "Log-in Successful!", "Success",
					 ...
					 * 
					 */
					searchAdminLogInFrame.dispose();
					
					new AllReservationsViaSearchEngine();
					/*
					 * 
					 * 
					 * 
					...
					JOptionPane.showMessageDialog(searchAdminLogInFrame,
		...
		searchAdminLogInFrame.getContentPane().add(registerWindow);
		...
		searchAdminLogInFrame.getContentPane().add(signinBtn);
		...
		searchAdminLogInFrame.getContentPane().add(registerBtn);
		...
		JPanel searchAdminLoginHeader = new JPanel();
		searchAdminLoginHeader.setBorder(new LineBorder(new Color(65, 105, 170)));
		searchAdminLoginHeader.setBounds(235, 16, 350, 32);
		searchAdminLogInFrame.getContentPane().add(searchAdminLoginHeader);
		searchAdminLoginHeader.setLayout(null);

		JLabel searchAdminLoginHeaderLbl = new JLabel("Search Engine Administrator Log-In");
		searchAdminLoginHeaderLbl.setBounds(10, 6, 334, 22);
		searchAdminLoginHeader.add(searchAdminLoginHeaderLbl);
		searchAdminLoginHeaderLbl.setFont(new Font("Lucida Grande", Font.BOLD, 18));

		JLabel windowBG = new JLabel("");
		windowBG.setIcon(new ImageIcon(SearchAdminLogin.class.getResource("/loginbackground.jpg")));
		windowBG.setBounds(0, 0, 730, 510);
		searchAdminLogInFrame.getContentPane().add(windowBG);
	}
