...
public class login {
	...
	private static JFrame customerLogInFrame;
	...
	private String filePath = "customerLogin.txt"; // File where account details will be stored
	...
	private void initialize() {
		...
		signinWindowBtn.addMouseListener(new MouseAdapter() {
			...
			// Uses verifyLogin function to check if username and password is valid in the text file. If valid details, logs in.
			public void mousePressed(MouseEvent e) {
				if (verifyLogin(signinWindowUsernameField.getText(), signinWindowPasswordField.getText(), filePath)) {
		...
		registerWindowBtn.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				if (checkUserExists(registerWindowUsernameField.getText(), filePath)) {
		...
		windowBG.setIcon(new ImageIcon(login.class.getResource("/loginWindow/loginbackground.jpg")));
	...
	public static boolean verifyLogin(String username, String password, String filepath) { // Checks username and password
		...
		try {
			...
			while (x.hasNext() && !found) {
				...
				if (tempUsername.trim().contentEquals(username.trim()) && tempPassword.trim().equals(password.trim())) {
					return found = true;
				}
			}
			x.close();
		} catch (Exception e) {
			JOptionPane.showMessageDialog(customerLogInFrame, "There was an error logging you in. Please try again.",
					"Error", JOptionPane.ERROR_MESSAGE);
		}
		return found;
	}

	public static boolean checkUserExists(String username, String filepath) {
		boolean found = false;
		String tempUsername = "";
		String tempPassword = "";
		...
		try {
			...
			while (x.hasNext() && !found) {
				...
				if (tempUsername.trim().contentEquals(username.trim())) {
					return found = true;
				}
			}
			x.close();
			...
			JOptionPane.showMessageDialog(customerLogInFrame,
					filepath + " does not exist. Creating the file now.", "Error", JOptionPane.ERROR_MESSAGE);
		}
		return found;
	}
}